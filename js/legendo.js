...
$(function(){
    if (window.innerWidth > 1159) {
        // PC
        $('#home-main-img .slider').slick({
            arrows: true,
            autoplay: true,
            //autoplaySpeed: 4000, // 
            autoplaySpeed: 19000, // Set only when first movie is available Change the number of milliseconds according to the length of the movie
            dots: true,
            fade: true,
            speed: 1500,
            infinite:true
        });
        $('#home-main-img .slider').on('afterChange', function(event, slick, currentSlide){
            if (currentSlide == 0) { // First slide and movie (starting from 0)
                theVideo.currentTime = 0;
                $('#home-main-img .slider').slick('slickPause');
                theVideo.play();
            }else{ // The slides are other than the above
                theVideo.pause();
                theVideo.currentTime = 0;
                $("#home-main-img .slider").slick('slickSetOption', 'autoplaySpeed', '4000');
            }
        });
        document.getElementById('theVideo').addEventListener('ended',myHandler,false);
        function myHandler(e) {
            $('#home-main-img .slider').slick('slickPlay');
        }       
    }else{
        // Mobile
        $('#home-main-img .slider').slick({
